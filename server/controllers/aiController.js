exports.generateNote = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, data: { title: titles[idx], content: contents[idx] } });
  } catch (err) { next(err); }
};
...
exports.summarize = async (req, res, next) => {
  try {
    const { url } = req.body;
    res.json({ success: true, data: { summary: 'This article discusses key insights and takeaways from the provided content. The main arguments center around practical strategies for improvement and actionable advice that can be implemented immediately.' } });
  } catch (err) { next(err); }
};
